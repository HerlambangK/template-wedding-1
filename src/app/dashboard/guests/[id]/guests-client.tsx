...
export default function GuestsClient({ invitation, guests: initialGuests }: Props) {
  ...
  const getWaMessage = (guestName: string, invitationLink: string) => {
    return [
      ...
      `*Jam:* ${invitation.akad_time || invitation.resepsi_time || "08:00"} - ${invitation.akad_end_time || invitation.resepsi_end_time || "10:30"} WIB`,
      `*Lokasi:* ${[invitation.akad_venue || invitation.resepsi_venue, invitation.akad_address || invitation.resepsi_address].filter(Boolean).join(", ") || "Nawasena Garden Resto & Ballroom, Jl. H. Agus Salim No.90, Pandean, Kec. Manguharjo, Kota Madiun, Jawa Timur 63129"}`,
      ``,
